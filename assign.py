...
def assign_chunk_faiss(i, index, embed_file, url_file):
    ...
    print("Assigning for file %d/4" % i)
...
def main():
    ...
    centroids_file = "static/add_centroids.npy"
    ...
    for i in range(NUM_CLUSTERS):
        f = open("static/clusters/cluster_%d.txt" % i, 'w')
        f.close()
    ...
    for idx in range(2):
        with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
            future_to_assignment = [executor.submit(assign_chunk_faiss, idx*2+j, index, embed_files[idx*2+j], url_files[idx*2+j]) for j in range(2)]
            for i, future in enumerate(concurrent.futures.as_completed(future_to_assignment)):
                ...
                for cluster in cluster_assignments:
                    with open("static/clusters/cluster_%d.txt" % cluster, 'a') as f:
    ...
    with open("static/clusters/cluster_sizes.txt", "w") as f:
        for cluster in cluster_assignments:
